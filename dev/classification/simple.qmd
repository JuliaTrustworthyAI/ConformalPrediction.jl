...
label_grid = [Int.(predict_mode(conf_mach.mach, [x1 x2]).refs) for x1 in X_grid[1], x2 in X_grid[2]]
p_grid = [MLJ.pdf.(predict(conf_mach.mach, [x1 x2]), 1) for x1 in X_grid[1], x2 in X_grid[2]]
C_grid = [sum([!ismissing(val) for (key,val) in predict(conf_mach, [x1 x2],coverage)[1]]) for x1 in X_grid[1], x2 in X_grid[2]]
```
