...
"""
function ConformalPrediction.score(conf_model::SimpleInductiveRegressor, ::Type{<:MLJFluxModel}, fitresult, X, y)
...
"""
function ConformalPrediction.score(conf_model::SimpleInductiveRegressor, ::Type{<:EitherEnsembleModel{<:MLJFluxModel}}, fitresult, X, y)
    X = permutedims(matrix(X))
    _chains = map(res -> res[1], fitresult.ensemble)
    ŷ = MLUtils.stack(map(chain -> chain(X), _chains)) |>
