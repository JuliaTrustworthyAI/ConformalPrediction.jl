...
using ConformalPrediction: BayesRegressor
using MLJEnsembles: EitherEnsembleModel
using MLJFlux: MLJFluxModel
using MLUtils

"""
    ConformalPrediction.score(conf_model::BayesRegressor, model::MLJFluxModel, fitresult, X, y)
...
"""
    ConformalPrediction.score(conf_model::BayesRegressor, ::Type{<:EitherEnsembleModel{<:MLJFluxModel}}, fitresult, X, y)

Overloads the `score` function for ensembles of `MLJFluxModel` types.
"""
function ConformalPrediction.score(
    conf_model::BayesRegressor,
